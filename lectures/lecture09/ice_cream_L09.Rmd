...
print("With a p-value of 0.557, we fail to reject the null hypothesis and conclude that students did not score differently on the puzzle and video scores")
...
print("Although the p-value (0.1418) is still greater than our significance level of 0.05 (and thus we still fail to reject the null hypothesis) we can see how different the sign test and the rank sum test are!")
...
print("We can see that the distribution of diff is skewed to the left. we have larger negative values than we have positive values, and this is the difference between the tests. If we had seen a normal distribution here, we likely would have seen a more similar p-value between the two tests!")
