...
# easy way - we can use wilcox.test!
...
We expected the puzzle to have an average score of 50. Does our data suggest this is true? Or did the students score higher than 50? 

```{r}

# sign test - manually
# how many individuals had puzzle score > 50?
df <- ice_cream %>%
    dplyr::mutate(more_than_50 = puzzle > 50)

pos <- sum(df$more_than_50)
neg <- 200-pos

# directional hypothesis: H_A: score > 50
# binomial distribution: n = 200, p = 0.5
pbinom(neg, 200, 0.5) ####################################### is this right?

```

Repeat the previous analysis comparing video scores to puzzle scores but this time use the Wilcox signed-rank test since the puzzle data is not normally distributed.

```{r}

wilcox.test(ice_cream$video, ice_cream$puzzle, paired = T)

# our results:
print("Our data suggests there is not a statistically significant difference between video scores and puzzle scores (p = 0.14).")

```

BONUS: Can you explain why your p-values were so different comparing video and puzzle scores using a parametric vs a nonparametric test?
